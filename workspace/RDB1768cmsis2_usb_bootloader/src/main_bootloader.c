...
// Provide access to RDB1768 LCD routines
#include "lcd.h"
...
/*****************************************************************
 * The reset button on the RDB1768 does not reset the audio codec.
 * Thus if a user application is loaded into flash that uses the
 * codec, and then reset is pressed, in conjunction with the button
 * to cause entry into the bootloader's usb mode, then it is
 * possible that whitenoise may be output through the speaker by
 * the codec. To prevent this define the symbol 'AUDIO_CODEC_RESET'
 * and the bootloader will then include code to manually reset the
 * codec during start up.
 ******************************************************************/
#define AUDIO_CODEC_RESET

#ifdef AUDIO_CODEC_RESET
#define AUDIO_INIT_DELAY 6000
#endif
