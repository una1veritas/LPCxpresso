...
volatile uint32_t InterruptLog[LOG_COUNT];
volatile uint32_t InterruptLogCount = 0;
...
uint32_t USB_WriteEP (uint32_t EPNum, uint8_t *pData, uint32_t cnt) {
  ...
  WrCmd   (CMD_SEL_EP(EPAdr(EPNum)));            /* select EP */ 
  n = RdCmdDat(DAT_SEL_EP(EPAdr(EPNum)));        /* get EP status */
  if (n & EP_SEL_ST) {                           /* check if EP is stalled */
    return (cnt);                                /* do not write to stalled EP */
  }
...
void USB_IRQHandler (void)
{
  ...
  LPC_USB->DevIntClr = disr;
#if USB_DEBUG
  InterruptLog[InterruptLogCount++] = disr;
  if ( InterruptLogCount >= LOG_COUNT )
  {
	InterruptLogCount = 0;
  }
#endif
